...
public class UpdateHelper {
    public func moveArtworkToFileManager(artworkRemoteInfo: ArtworkRemoteInfo, asyncCompanion: CoreDataCompanion) {
        autoreleasepool {
    ...
    public func moveEmbeddedArtworkToFileManager(embeddedArtworkOwner: AbstractPlayable, asyncCompanion: CoreDataCompanion) {
        autoreleasepool {
    ...
    public func movePlayableToFileManager(playable: AbstractPlayable, asyncCompanion: CoreDataCompanion) {
    ...
    public func getPlayableDataAndDeleteCoreDataFile(playable: AbstractPlayable, asyncCompanion: CoreDataCompanion) -> Data? {
        var data: Data?
        autoreleasepool {
...
public class LibraryUpdater {
    ...
    @MainActor private let log = OSLog(subsystem: "Amperfy", category: "BackgroundSyncer")
    ...
    @MainActor public func performSmallBlockingLibraryUpdatesIfNeeded() {
    ...
    @MainActor public func performLibraryUpdateWithStatus(notifier: LibraryUpdaterCallbacks) async throws {
        ...
        if self.storage.librarySyncVersion < .v17 {
            ...
            try await self.extractBinaryDataToFileManager(notifier: notifier)
        ...
        if self.storage.librarySyncVersion < .v18 {
            ...
            try await self.denormalizeCount(notifier: notifier)
        ...
        if self.storage.librarySyncVersion < .v19 {
            ...
            try await self.sortPlaylistItems(notifier: notifier)
            os_log("Perform blocking library update (DONE): Sort playlist items", log: self.log, type: .info)
        }
    }
    
    @MainActor private func updateArtworkIdStructure() {
    ...
    @MainActor private func updateAlphabeticSectionInitial() {
    ...
    @MainActor private func updatePlaylistArtworkItems() {
        let playlists = storage.main.library.getPlaylists()
        playlists.forEach{
    ...
    @MainActor private func extractBinaryDataToFileManager(notifier: LibraryUpdaterCallbacks) async throws {
        ...
        try await storage.async.perform { asyncCompanion in
            ...
            for artworkRemoteInfo in artworkRemoteInfos {
                UpdateHelper().moveArtworkToFileManager(artworkRemoteInfo: artworkRemoteInfo, asyncCompanion: asyncCompanion)
        ...
        try await storage.async.perform { asyncCompanion in
            ...
            for embeddedArtworkOwner in embeddedArtworkOwners {
                UpdateHelper().moveEmbeddedArtworkToFileManager(embeddedArtworkOwner: embeddedArtworkOwner, asyncCompanion: asyncCompanion)
                asyncCompanion.library.saveContext()
                notifier.tickOperation()
            }
        }
        ...
        try await storage.async.perform { asyncCompanion in
            ...
            for cachedSong in cachedSongs {
                UpdateHelper().movePlayableToFileManager(playable: cachedSong, asyncCompanion: asyncCompanion)
                asyncCompanion.library.saveContext()
                notifier.tickOperation()
            }
            for cachedEpisode in cachedEpisodes {
                UpdateHelper().movePlayableToFileManager(playable: cachedEpisode, asyncCompanion: asyncCompanion)
                asyncCompanion.library.saveContext()
                notifier.tickOperation()
            }
    ...
    @MainActor private func sortPlaylistItems(notifier: LibraryUpdaterCallbacks) async throws {
    ...
    @MainActor  private func denormalizeCount(notifier: LibraryUpdaterCallbacks) async throws {
