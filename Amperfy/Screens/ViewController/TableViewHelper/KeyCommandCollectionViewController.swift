...
class CollectionViewKeyCommandsController {
    ...
    func moveFocusToPrevious() {
        ...
        if focussedRow == 0 {
            ...
            if prevSection < 0 {
                ...
                self.focussedIndexPath = IndexPath(item: collectionView.numberOfItems(inSection: prevSection) - 1, section: prevSection)
            }
        } else {
            self.focussedIndexPath = IndexPath(item: focussedRow - 1, section: focussedSection)
        }
    ...
    private var lastIndexPath: IndexPath? {
        ...
        if lastSectionWithRows >= 0 {
            return IndexPath(item: collectionView.numberOfItems(inSection: lastSectionWithRows) - 1, section: lastSectionWithRows)
        } else {
            return nil
        }
    }
