...
extension PopupPlayerVC {
    ...
    func changeDisplayStyleVisually(to displayStyle: PlayerDisplayStyle, animated: Bool = true) {
        ...
        if animated {
            ...
            artworkSourceFrame = limitSizeToInsideThePlaceholder(targetFrame: artworkSourceFrame, placeholderFrame: largePlayerPlaceholderView.frame)
            var artworkTargetFrame = view.convert(artworkToApper.frame, from: containerToApper)
            artworkTargetFrame = limitSizeToInsideThePlaceholder(targetFrame: artworkTargetFrame, placeholderFrame: largePlayerPlaceholderView.frame)
            // 3. Create fake image and animate it
            animateArtwork(image: largeCurrentlyPlayingView?.artworkImage.image, sourceView: artworkToDisapper, targetView: artworkToApper, sourceFrame: artworkSourceFrame, targetFrame: artworkTargetFrame)
            
            favoriteToApper.layoutIfNeeded()
            var favoriteSourceFrame = view.convert(favoriteToDisapper.frame, from: detailsContainerToDisapper)
            favoriteSourceFrame = limitSizeToInsideThePlaceholder(targetFrame: favoriteSourceFrame, placeholderFrame: largePlayerPlaceholderView.frame)
            var favoriteTargetFrame = view.convert(favoriteToApper.frame, from: detailsContainerToApper)
            favoriteTargetFrame = limitSizeToInsideThePlaceholder(targetFrame: favoriteTargetFrame, placeholderFrame: largePlayerPlaceholderView.frame)
            animateFavorite(sourceView: favoriteToDisapper, targetView: favoriteToApper, sourceFrame: favoriteSourceFrame, targetFrame: favoriteTargetFrame)
            
            optionsToApper.layoutIfNeeded()
            var optionsSourceFrame = view.convert(optionsToDisapper.frame, from: detailsContainerToDisapper)
            optionsSourceFrame = limitSizeToInsideThePlaceholder(targetFrame: optionsSourceFrame, placeholderFrame: largePlayerPlaceholderView.frame)
            var optionsTargetFrame = view.convert(optionsToApper.frame, from: detailsContainerToApper)
            optionsTargetFrame = limitSizeToInsideThePlaceholder(targetFrame: optionsTargetFrame, placeholderFrame: largePlayerPlaceholderView.frame)
            animateOptions(sourceView: optionsToDisapper, targetView: optionsToApper, sourceFrame: optionsSourceFrame, targetFrame: optionsTargetFrame)
            
            viewToDisapper.isHidden = false
            viewToApper.isHidden = false
            UIView.animate(withDuration: Self.displaStyleAnimationDuration*2/3, delay: 0, animations: ({
                viewToDisapper.alpha = 0.0
            }), completion: nil)
            UIView.animate(withDuration: Self.displaStyleAnimationDuration*2/3, delay: Self.displaStyleAnimationDuration/3, animations: ({
    ...
    private func limitSizeToInsideThePlaceholder(targetFrame: CGRect, placeholderFrame: CGRect) -> CGRect {
        if targetFrame.origin.y < placeholderFrame.origin.y - targetFrame.height {
            return CGRect(x: targetFrame.origin.x, y: placeholderFrame.origin.y - targetFrame.height, width: targetFrame.width, height: targetFrame.height)
        } else if targetFrame.origin.y > placeholderFrame.origin.y + placeholderFrame.height {
            return CGRect(x: targetFrame.origin.x, y: placeholderFrame.origin.y + placeholderFrame.height, width: targetFrame.width, height: targetFrame.height)
        } else {
            return targetFrame
        }
    }
    
    private func animateArtwork(image: UIImage?, sourceView: UIView, targetView: UIView, sourceFrame: CGRect, targetFrame: CGRect) {
    ...
    private func animateFavorite(sourceView: UIView, targetView: UIView, sourceFrame: CGRect, targetFrame: CGRect) {
    ...
    private func animatePlayStyleObject(object: UIView, sourceView: UIView, targetView: UIView, sourceFrame: CGRect, targetFrame: CGRect) {
