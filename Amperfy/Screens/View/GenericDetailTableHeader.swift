...
class GenericDetailTableHeader: UIView {
    ...
    @IBOutlet weak var playShuffleInfoContainerView: UIView!
    
    @IBOutlet weak var titlePlayButtonContainerHeightConstraint: NSLayoutConstraint!
    ...
    private static let titlePlayButtonContainerHeightWithoutButtons: CGFloat = titlePlayButtonContainerHeightCompact - LibraryElementDetailTableHeaderView.frameHeight
    ...
    public static func createTableHeader(configuration: DetailHeaderConfiguration) -> GenericDetailTableHeader? {
        configuration.tableView.tableHeaderView = UIView(frame: CGRect(x: 0, y: 0, width: configuration.rootView.view.bounds.size.width, height: GenericDetailTableHeader.frameHeight(traitCollection: configuration.rootView.traitCollection)))
    ...
    override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        ...
        if traitCollection.horizontalSizeClass == .compact {
            if config.playShuffleInfoConfig == nil {
                titlePlayButtonContainerHeightConstraint.constant = Self.titlePlayButtonContainerHeightWithoutButtons
                height -= (Self.titlePlayButtonContainerHeightCompact - Self.titlePlayButtonContainerHeightWithoutButtons)
            } else {
                titlePlayButtonContainerHeightConstraint.constant = Self.titlePlayButtonContainerHeightCompact
            }
        }
        if config.descriptionText != nil {
            height += GenericDetailTableHeader.frameHeightForDescription
        }
        config.tableView.tableHeaderView?.frame = CGRect(x: 0, y: 0, width: rootView.view.bounds.size.width, height: height)
