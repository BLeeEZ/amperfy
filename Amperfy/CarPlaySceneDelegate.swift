...
class CarPlaySceneDelegate: UIResponder, CPTemplateApplicationSceneDelegate {
    ...
    func createCachedSections() -> [CPListSection] {
        let librarySections = [
            CPListSection(items: [
                ...
                createLibraryItem(text: "Favorites", icon: UIImage.heartFill, sectionToDisplay: songsFavoriteCachedSection)
            ], header: "Cached Songs", sectionIndexTitle: nil),
            CPListSection(items: [
                createPlayRandomAlbumsItem(onlyCached: true),
                createLibraryItem(text: "Favorites", icon: UIImage.heartFill, sectionToDisplay: albumsFavoriteCachedSection),
                createLibraryItem(text: "Newest", icon: UIImage.albumNewest, sectionToDisplay: albumsNewestCachedSection),
                createLibraryItem(text: "Recently Played", icon: UIImage.albumRecent, sectionToDisplay: albumsRecentCachedSection)
            ], header: "Cached Albums", sectionIndexTitle: nil),
            CPListSection(items: [
                createLibraryItem(text: "Favorites", icon: UIImage.heartFill, sectionToDisplay: artistsFavoriteCachedSection)
    ...
    func createLibraryItem(text: String, icon: UIImage, sectionToDisplay: CPListTemplate) -> CPListItem {
        let item =  CPListItem(text: text, detailText: nil, image: UIImage.createArtwork(with: icon, iconSizeType: .small, theme: appDelegate.storage.settings.themePreference, lightDarkMode: traits.userInterfaceStyle.asModeType, switchColors: true).carPlayImage(carTraitCollection: traits), accessoryImage: nil, accessoryType: .disclosureIndicator)
    ...
    func createContinePlayingItems() -> [CPListItem] {
        ...
        if appDelegate.player.musicItemCount > 0 {
            let item = CPListItem(text: "Music", detailText: "", image: UIImage.createArtwork(with: UIImage.musicalNotes, iconSizeType: .small, theme: appDelegate.storage.settings.themePreference, lightDarkMode: traits.userInterfaceStyle.asModeType, switchColors: true).carPlayImage(carTraitCollection: traits), accessoryImage: nil, accessoryType: .none)
            item.handler = { [weak self] item, completion in
                guard let `self` = self else { completion(); return }
        ...
        if appDelegate.player.podcastItemCount > 0 {
            let item = CPListItem(text: "Podcasts", detailText: "", image: UIImage.createArtwork(with: UIImage.podcast, iconSizeType: .small, theme: appDelegate.storage.settings.themePreference, lightDarkMode: traits.userInterfaceStyle.asModeType, switchColors: true).carPlayImage(carTraitCollection: traits), accessoryImage: nil, accessoryType: .none)
            item.handler = { [weak self] item, completion in
                guard let `self` = self else { completion(); return }
    ...
    func createPlayRandomSongsItem(onlyCached: Bool) -> CPListItem {
        let img = UIImage.createArtwork(with: UIImage.shuffle, iconSizeType: .small, theme: appDelegate.storage.settings.themePreference, lightDarkMode: traits.userInterfaceStyle.asModeType, switchColors: true).carPlayImage(carTraitCollection: traits)
        let item =  CPListItem(text: "Play Random\(onlyCached ? " Cached" : "") Songs", detailText: nil, image: img, accessoryImage: nil, accessoryType: .disclosureIndicator)
        item.handler = { [weak self] item, completion in
            guard let `self` = self else { completion(); return }
            let songs = self.appDelegate.storage.main.library.getRandomSongs(onlyCached: onlyCached || isOfflineMode)
            let playContext = PlayContext(name: "Random\(onlyCached ? " Cached" : "") Songs", playables: songs)
    ...
    func createPlayRandomAlbumsItem(onlyCached: Bool) -> CPListItem {
        let img = UIImage.createArtwork(with: UIImage.shuffle, iconSizeType: .small, theme: appDelegate.storage.settings.themePreference, lightDarkMode: traits.userInterfaceStyle.asModeType, switchColors: true).carPlayImage(carTraitCollection: traits)
        let item =  CPListItem(text: "Play Random\(onlyCached ? " Cached" : "") Albums", detailText: nil, image: img, accessoryImage: nil, accessoryType: .disclosureIndicator)
        item.handler = { [weak self] item, completion in
            guard let `self` = self else { completion(); return }
            let randomAlbums = self.appDelegate.storage.main.library.getRandomAlbums(count: 5, onlyCached: onlyCached || isOfflineMode)
            var songs = [AbstractPlayable]()
            randomAlbums.forEach { songs.append(contentsOf: $0.playables.filterCached(dependigOn: onlyCached || self.isOfflineMode)) }
            let playContext = PlayContext(name: "Random\(onlyCached ? " Cached" : "") Albums", playables: songs)
    ...
    private func createArtistItems(from fetchedController: BasicFetchedResultsController<ArtistMO>?, onlyCached: Bool) -> [CPListTemplateItem] {
        ...
        for index in 0...(CPListTemplate.maximumSectionCount-1) {
            ...
            let detailTemplate = self.createDetailTemplate(for: entity, onlyCached: onlyCached)
            items.append(detailTemplate)
        }
        return items
    }
    private func createAlbumItems(from fetchedController: BasicFetchedResultsController<AlbumMO>?, onlyCached: Bool) -> [CPListTemplateItem] {
        var items = [CPListTemplateItem]()
        guard let fetchedController = fetchedController else { return items }
        for index in 0...(CPListTemplate.maximumSectionCount-1) {
            guard let entity = fetchedController.getWrappedEntity(at: index) else { break }
    ...
    private func createDetailTemplate(for artist: Artist, onlyCached: Bool) -> CPListItem {
        ...
        section.handler = { [weak self] item, completion in
            ...
            albumItems.append(self.createPlayShuffledListItem(playContext: PlayContext(containable: artist, playables: artist.playables.filterCached(dependigOn: onlyCached || self.isOfflineMode))))
            albumItems.append(self.createDetailAllSongsTemplate(for: artist, onlyCached: onlyCached))
            let artistAlbums = self.appDelegate.storage.main.library.getAlbums(whichContainsSongsWithArtist: artist, onlyCached: onlyCached || self.isOfflineMode).prefix(LibraryStorage.carPlayMaxElements)
            for album in artistAlbums {
                let listItem = self.createDetailTemplate(for: album, onlyCached: onlyCached)
    ...
    private func createDetailAllSongsTemplate(for artist: Artist, onlyCached: Bool) -> CPListItem {
        let section = CPListItem(text: "All Songs", detailText: nil, image: UIImage.createArtwork(with: UIImage.musicalNotes, iconSizeType: .small, theme: appDelegate.storage.settings.themePreference, lightDarkMode: traits.userInterfaceStyle.asModeType, switchColors: true).carPlayImage(carTraitCollection: traits), accessoryImage: nil, accessoryType: .disclosureIndicator)
        section.handler = { [weak self] item, completion in
            guard let `self` = self else { completion(); return }
            var songItems = [CPListItem]()
            songItems.append(self.createPlayShuffledListItem(playContext: PlayContext(containable: artist, playables: artist.playables.filterCached(dependigOn: onlyCached || self.isOfflineMode))))
            let artistSongs = artist.playables.filterCached(dependigOn: onlyCached || self.isOfflineMode).sortByTitle().prefix(LibraryStorage.carPlayMaxElements)
    ...
    private func createDetailTemplate(for album: Album, onlyCached: Bool) -> CPListItem {
        ...
        section.handler = { [weak self] item, completion in
            guard let `self` = self else { completion(); return }
            var songItems = [CPListItem]()
            songItems.append(self.createPlayShuffledListItem(playContext: PlayContext(containable: album, playables: album.playables.filterCached(dependigOn: onlyCached || self.isOfflineMode))))
            let albumSongs = album.playables.filterCached(dependigOn: onlyCached || self.isOfflineMode).prefix(LibraryStorage.carPlayMaxElements)
    ...
    @objc private func refreshSort() {
        ...
        if artistsFavoritesCachedFetchController?.sortType != appDelegate.storage.settings.artistsSortSetting {
            os_log("CarPlay: RefreshSort: ArtistsFavoritesCachedFetchController", log: self.log, type: .info)
        ...
        if templates.contains(albumsFavoriteCachedSection), albumsFavoritesCachedFetchController?.sortType != appDelegate.storage.settings.albumsSortSetting {
            os_log("CarPlay: RefreshSort: AlbumsFavoritesCachedFetchController", log: self.log, type: .info)
